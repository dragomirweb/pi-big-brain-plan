...
import type { PlannerDetails } from "./subagent.ts";
...
function emptyUsage(): PlannerDetails["usage"] {
  return { input: 0, output: 0, cacheRead: 0, cacheWrite: 0, cost: 0, contextTokens: 0, turns: 0 };
}

function extractUsage(details: JsonObject | undefined): PlannerDetails["usage"] {
...
export function runViaBridge(
  ...
  return new Promise<AgentToolResult<PlannerDetails> | null>((resolve) => {
    let responded = false;
    ...
    const unsubResponse = pi.events.on(SLASH_RESPONSE_EVENT, (data: unknown) => {
      ...
      resolve({ content: [{ type: "text", text }], details: { usage } });
    });
    ...
    if (signal) {
      const onAbort = () => {
        ...
        resolve({
          content: [{ type: "text", text: "Planning aborted." }],
          details: { usage: emptyUsage() },
        });
      };
